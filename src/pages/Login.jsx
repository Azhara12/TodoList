  ...
  return (
    <div className="hidden md:flex md:w-1/2 lg:w-[55%] relative items-center justify-center bg-gradient-to-br from-[#1b1530] via-[#0f0e13] to-[#0a0812] overflow-hidden p-12 border-r border-[#2e2646]/30">
      ...
      <div className="relative z-10 w-full max-w-lg flex flex-col items-center text-center">
        ...
        <span className="text-xs uppercase tracking-[0.25em] text-violet-400 font-semibold mb-3">
        ...
        <p className="mt-4 text-sm text-slate-400 max-w-sm leading-relaxed">
          Create tasks, organize workflows, and track your daily progress in a
...
function Toast({ message, type }) {
  ...
  return (
    <div
      className={`mb-5 px-4 py-3 rounded-xl border text-xs font-medium ${colors}`}
    >
      {message}
    </div>
  );
    ...
    try {
      ...
      setTimeout(() => {
        if (typeof setLogged === "function") {
          ...
          console.error(
            "Error: setLogged is not passed correctly from Parent Component!",
          );
        }
      }, 1000);
    } catch (error) {
    ...
    <div className="w-full max-w-md">
      ...
      <form onSubmit={handleLogin} className="space-y-5">
        ...
        <div className="flex items-center">
          ...
          <label
            htmlFor="remember_me"
            className="ml-2 text-xs text-slate-400 cursor-pointer select-none"
          >
            Keep me signed in
          </label>
        </div>
...
function ForgotPasswordScreen({ goTo, registeredUser, setRegisteredUser }) {
  ...
  function handleReset(e) {
    ...
    if (
      ...
      registeredUser.password !== oldPassword
    ) {
      setToast({
        type: "error",
...
function RegisterScreen({ goTo, setRegisteredUser }) {
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      setToast({
        type: "success",
        message: `Account created for ${data.name}! Redirecting to sign in…`,
      });
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-400 mt-2">
          Join thousands of people staying organized every day.
        </p>
      </div>
        ...
        <div>
          ...
          {password.length > 0 && (
            <div className="mt-2 flex items-center gap-3">
              ...
              <span
                className={`text-xs font-medium ${strength <= 1 ? "text-red-400" : strength === 2 ? "text-yellow-400" : strength === 3 ? "text-blue-400" : "text-emerald-400"}`}
              >
